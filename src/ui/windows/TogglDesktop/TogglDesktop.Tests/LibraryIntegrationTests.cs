...
namespace TogglDesktop.Tests
{
    public class LibraryIntegrationTests : IClassFixture<LibraryFixture>
    {
        ...
        public void EditShouldTriggerOnTimeEntryEditor()
        {
            ...
            using (Toggl.OnTimeEntryEditor.Subscribe(Assertion))
            {
                Toggl.Edit(_firstTimeEntryGuid, false, "description");
            }

            void Assertion((bool open, Toggl.TogglTimeEntryView timeEntry, string focusedFieldName) x)
            {
                Assert.True(x.open);
                Assert.Equal(x.timeEntry.GUID, _firstTimeEntryGuid);
                Assert.Equal("description", x.focusedFieldName);
        ...
        public void EditPreferencesShouldOpenSettings()
        {
            ...
            using (Toggl.OnSettings.Subscribe(Assertion))
            {
                Toggl.EditPreferences();
            }

            void Assertion((bool open, Toggl.TogglSettingsView settings) x)
            {
                Assert.True(x.open);
                assertionExecuted = true;
            }
        ...
        public void SetSettingsShouldSaveSettings(bool autotrack, bool pomodoro)
        {
            ...
            using (Toggl.OnSettings.Subscribe(ReadCurrentSettings))
            {
                Toggl.EditPreferences();
            }

            void ReadCurrentSettings((bool open, Toggl.TogglSettingsView settings) x) => currentSettings = x.settings;

            var savedAutotrack = false;
            var savedPomodoro = false;

            using (Toggl.OnSettings.Subscribe(ReadSavedSettings))
            {
            ...
            void ReadSavedSettings((bool open, Toggl.TogglSettingsView settings) x)
            {
                savedAutotrack = x.settings.Autotrack;
                savedPomodoro = x.settings.Pomodoro;
            }
        ...
        {
            var assertionExecuted = false;
            using (Toggl.OnLogin.Subscribe(Assertion))
            {
                Assert.True(Toggl.Logout());
            }

            void Assertion((bool open, ulong userId) x)
            {
                Assert.True(x.open);
                Assert.Equal(0ul, x.userId);
                assertionExecuted = true;
            }
            Assert.True(assertionExecuted);
        }
        ...
        {
            var assertionExecuted = false;
            using (Toggl.OnClientSelect.Subscribe(Assertion))
            {
